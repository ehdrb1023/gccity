...
describe('parseResolutionBody — 외부 기관으로 넘어간 건', () => {
  ...
  /**
   * 실측 2026-08-24 — 이 한 줄에 함정이 둘 있었다.
   *   ① 기관명에 마침표가 있다 ("LH 의왕.과천 사업소")
   *   ② 부서명이 여러 낱말이다 ("과천시청 공원녹지과 정원도시팀")
   * 둘 다 못 읽어 화면에 "배분 전" 으로 떴다 — "아직 안 정해졌다" 와 겉보기가 같아서
   * 틀린 것을 틀린 줄 모르는 상태였다.
   */
  it('★ 마침표가 든 기관명과 여러 낱말 부서명을 함께 읽는다', () => {
    const p = parseResolutionBody(
      '본 민원을 과천시청 공원녹지과 정원도시팀에서 담당 기관인 LH 의왕.과천 사업소에 문의한 결과 근린 공원 3부지는 2026년 12월말 준공 예정이였지만 맹꽁이가 발견되어 부득이하게 공사가 중단된 상황이다 라는 회신을 받았습니다.',
    );
    expect(p.department).toBe('과천시청 공원녹지과 정원도시팀');
