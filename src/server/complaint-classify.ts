...
export type ParsedBody = {
  ...
  /**
   * 배분 부서 — **담당 기관 안**에서 이 민원을 맡은 곳. "담당 부서인 X에 문의한 결과" 꼴이거나,
   * 외부로 넘긴 건이면 "X에서 담당 기관인 …에게" 의 X 다.
   */
  department: string | null;
  /**
   * 회신 기관 — **담당 기관 밖**에서 답을 준 곳(시공사·공기업 등). "담당 기관인 Y에게" 꼴이다.
   *
   * ★ 부서와 한 칸에 담지 말 것. 부서별 집계를 낼 때 내부 과·팀과 외부 기관이 한 줄에
...
/** ③ 의 뒤쪽 — 담당 기관 밖에서 답을 준 곳. */
const AGENCY = new RegExp(`담당\\s*기관(?:인|는|:)?\\s*(${ORG}{2,60}?)\\s*(?:에서|에게|에|으로|이)\\s+`);

/**
 * 회신문에 나오는 **기관 이름**.
 *
 * ★ 기관마다 다르므로 하드코딩하지 않는다. 기본값은 한국 지자체 일반형이고,
 *   오탐이 잦으면 `CIVIC_OFFICE_NAMES` 에 정확한 이름을 쉼표로 적어 좁힌다
 *   (예: `CIVIC_OFFICE_NAMES=과천시청,과천시`).
 *
 * ★ env 에서 **정규식이 아니라 이름 목록**을 받는다. 정규식을 그대로 받으면 운영자가
 *   실수로 넣은 한 줄이 회신문마다 도는 백트래킹 폭탄이 된다.
 */
const escapeRe = (v: string) => v.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

const OFFICE =
  (process.env.CIVIC_OFFICE_NAMES ?? '')
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean)
    .map(escapeRe)
    .join('|') || '[가-힣]{2,8}(?:시청|군청|구청|도청|교육청|시|군|구)';

/*
 * ③ 의 앞쪽 — 외부로 넘긴 **기관 내부 부서**.
 *
 * ★ 부서명이 여러 낱말이다("○○시청 공원녹지과 정원도시팀"). 한 낱말만 잡으면
 *   "정원도시팀" 처럼 소속이 잘려 나가고, 반대로 아무 낱말이나 이어 붙이면
 *   "본 민원을" 같은 앞머리가 딸려 온다. 그래서 **기관 이름으로 시작하는 덩어리**를 먼저 보고,
 *   그게 없을 때만 조직 꼬리(과·팀·실…)로 끝나는 한 낱말로 떨어진다.
 */
const VIA_DEPARTMENT = new RegExp(
  `((?:${OFFICE})(?:\\s+[가-힣A-Za-z0-9·.]+?){0,3}|[가-힣A-Za-z0-9·.]{2,15}(?:과|팀|실|담당관|사업소|본부|센터))\\s*에서\\s+담당\\s*기관`,
);

/*
 * 말머리 없이 바로 부서를 적는 회신. 반드시 **기관 이름으로 시작**하고 조직 이름이 한 낱말
 * 이상 이어져야 한다 — 그러지 않으면 "○○시는 …" 같은 평범한 문장을 부서로 읽는다.
 *
 * ★ 낱말이 **게으르다**(`+?`). 탐욕이면 조사(`에서`)가 낱말에 붙어 있을 때 그 낱말을 통째로
 *   삼키고 다음 낱말까지 먹는다 — "…주차지도팀에서 관리단에" 가 통째로 부서가 된다.
 *   반복(`{1,3}`)만 게으르게 해서는 안 고쳐진다. 낱말 안에서 멈출 수 있어야 조사가
 *   처음 나오는 자리에서 끊긴다. `+` 로 되돌리지 말 것.
 *
 * 실측: 옛 규칙은 과천 회신문이 우연히 `{1,3}` 경계 밖이라 통과했을 뿐, 같은 문장에서
 * 낱말이 하나만 적으면 똑같이 넘겨 먹었다.
 */
const BARE_DEPARTMENT = new RegExp(
  `((?:${OFFICE})(?:\\s+[가-힣A-Za-z0-9·.]+?){1,3})\\s*(?:에서|에게|에|으로|이)\\s`,
);
...
export function parseResolutionBody(body: string): ParsedBody {
  ...
  if (agency) {
    ...
    // 외부로 넘긴 건이면 배분 부서는 그 앞에 적힌 기관 조직이다
   ...
   *   그래서 마지막으로 **기관 이름으로 시작하는 조직 덩어리**를 한 번 더 찾는다.
